...
RegisterNetEvent(
    "Coinflip:sendCoinflipTableData",
    function(C)
        f = C
    end
)
Citizen.CreateThread(
    function()
        Wait(1000)
        TriggerServerEvent("Coinflip:requestCoinflipTableData")
        for q = 0, #c.coinflipTables do
            local D = c.coinflipTables[q]
            local E = tvRP.loadModel(D.tableModel)
            ...
            if d then
                ...
                if
                    not l and IsControlJustPressed(0, 202) and not g and
                        not RageUI.Visible(RMenu:Get("versecoinflip", "confirm"))
                 then
                    local L = tvRP.loadAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                    TaskPlayAnim(tvRP.getPlayerPed(), L, "sit_exit_left", 1.0, 1.0, 2500, 0, 0.0, false, false, false)
                    RemoveAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                    d = false
                    e = true
                    showHowToCoinflip(false)
                    TriggerServerEvent("Coinflip:cancelCoinflip", true)
                    TriggerServerEvent("Coinflip:leaveCoinflipTable")
                    h = {}
                    m = false
                    SetTimeout(
                        5000,
            ...
            if not d then
                if a ~= -1 and b < 2 then
                    if IsControlJustPressed(0, 38) then
                        if f[a] == false then
                            TriggerServerEvent("Coinflip:requestSitAtCoinflipTable", a)
                        end
                    end
                end
            end
...
function goToCoinflipSeat(o)
    ...
    TaskGoStraightToCoord(tvRP.getPlayerPed(), S.x, S.y, S.z, 1.0, 5000, T.z, 0.01)
    Wait(500)
    local U = v(o)
    local V = A(o)
...
function showHowToCoinflip(X)
    ...
    RageUI.Visible(RMenu:Get("versecoinflip", "instructions"), X)
end
function showCoinflipMenu(X)
    RageUI.ActuallyCloseAll()
    RageUI.Visible(RMenu:Get("versecoinflip", "mainmenu"), X)
end
RMenu.Add(
    "versecoinflip",
    "instructions",
    RageUI.CreateMenu(
        "",
        "~b~COINFLIP",
        tvRP.getRageUIMenuWidth(),
        tvRP.getRageUIMenuHeight(),
        "verse_coinflip",
        "verse_coinflip"
    )
)
...
RMenu.Add(
    ...
    RageUI.CreateSubMenu(
        RMenu:Get("versecoinflip", "mainmenu"),
        "",
        "~b~COINFLIP",
        tvRP.getRageUIMenuWidth(),
        tvRP.getRageUIMenuHeight(),
...
RageUI.CreateWhile(
    ...
    function()
        RageUI.IsVisible(
            RMenu:Get("versecoinflip", "instructions"),
            true,
            false,
            true,
            function()
        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.ButtonWithStyle(
                    ...
                    function(Y, Z, _)
                        if _ then
                            local a0 = getGenericTextInput("Bet Amount")
                            if tonumber(a0) then
                                a0 = tonumber(a0)
                                if a0 > 0 then
                                    TriggerServerEvent("Coinflip:proposeCoinflip", a0)
                                end
                            end
                        end
                    end
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(Y, Z, _)
                                if _ then
                                    ...
                                    if M.user_id == tvRP.getUserId() then
                                        k = true
                                    else
                                        k = false
                                    end
                            ...
                            RMenu:Get("versecoinflip", "confirm")
                        )
                    end
                else
            ...
            function()
                ...
                if k then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(Y, Z, _)
                            if _ then
                                TriggerServerEvent("Coinflip:cancelCoinflip")
                            end
                        end,
                        RMenu:Get("versecoinflip", "mainmenu")
                    )
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(Y, Z, _)
                            if _ then
                                TriggerServerEvent("Coinflip:acceptCoinflip", i)
                            end
                        end,
                        RMenu:Get("versecoinflip", "mainmenu")
                    )
...
RegisterNetEvent(
    "Coinflip:addCoinflipProposal",
    function(a1, M)
        h[a1] = M
    end
)
RegisterNetEvent(
    "Coinflip:cancelCoinflipBet",
    function(a1)
        h[a1] = nil
    end
)
RegisterNetEvent(
    "Coinflip:clearCoinflipBets",
    function(a1)
        h = {}
    end
)
RegisterNetEvent("Coinflip:coinflipOutcome",function(a4, a5)
        l = true
        ...
        SetTimeout(10000,function()
                a7 = true
                if a4 then
                    tvRP.notify("~g~ You won £" .. getMoneyStringFormatted(a5.amount) .. "!")
                    SendNUIMessage({transactionType = "playCasinoWin"})
                else
                    tvRP.notify("~r~ You lost £" .. getMoneyStringFormatted(a5.amount) .. "!")
